...
ret(getNext(ret, 6)) = computePeakStatisticsOverBiggestPeaks(sortedPeaks, sortedLocs, length(sortedPeaks));

% 100 largest peaks statistics
ret(getNext(ret, 6)) = computePeakStatisticsOverBiggestPeaks(sortedPeaks, sortedLocs, 100);

% 50 largest peaks statistics
ret(getNext(ret, 6)) = computePeakStatisticsOverBiggestPeaks(sortedPeaks, sortedLocs, 50);

% 10 largest peaks statistics
ret(getNext(ret, 6)) = computePeakStatisticsOverBiggestPeaks(sortedPeaks, sortedLocs, 10);

% absolute value of signal
absData = abs(data);

% peaks of abs ampltude of .10 relative prominance
ret(getNext(ret, 3)) = findPeakDistancesWithMinProminance(absData, sampleFrequency, .1);

% peaks of abs ampltude of .15 relative prominance
ret(getNext(ret, 3)) = findPeakDistancesWithMinProminance(absData, sampleFrequency, .15);

% peaks of abs ampltude of .3 relative prominance
ret(getNext(ret, 3)) = findPeakDistancesWithMinProminance(absData, sampleFrequency, .3);

% peaks of abs ampltude of .05 relative prominance
ret(getNext(ret, 3)) = findPeakDistancesWithMinProminance(absData, sampleFrequency, .05);

% peaks of abs ampltude of .5 relative prominance
ret(getNext(ret, 3)) = findPeakDistancesWithMinProminance(absData, sampleFrequency, .5);
